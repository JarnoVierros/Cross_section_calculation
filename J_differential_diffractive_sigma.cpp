...
const int integration_calls = 4*20000000;//20 000 000
...
const int predictions = 3;
double x_values[predictions] = {0.004,0.010,0.018};
double Q2_values[predictions] = {35,35,35};
double beta_values[predictions] = {0.25,0.10,0.04};
...
double L_integrand(double r1, double r2, double b1, double b2, double r1bar, double r2bar, double z, double Q2, double x, double beta) {
  if (z*(1-z)*(Q2*(1/beta-1)-4*m_f*m_f)-m_f*m_f < 0) {
  ...
  double integrand = gsl_sf_bessel_J0(sqrt(z*(1-z)*(Q2*(1/beta-1)-4*m_f*m_f)-m_f*m_f)*sqrt(gsl_pow_2(r1-r1bar)+gsl_pow_2(r2-r2bar)))
  *z*(1-z)
...
void integrate_for_L_sigma(thread_par_struct par) {
    ...
    double z_min = (1-sqrt(1-4*m_f*m_f/(par.Q2*(1/par.beta-1)-4*m_f*m_f)))/2;
    double z_max = (1+sqrt(1-4*m_f*m_f/(par.Q2*(1/par.beta-1)-4*m_f*m_f)))/2;
...
int main() {
  ...
  vector<double> x_pom_F2_values, delta_stat_values, delta_sys_values;
  ...
  if (diffraction_dipamp) {
    ...
    filename_end = "_diffractivedipamp" + filename_end;
  } else {
  ...
  thread L_integration_threads[predictions], T_integration_threads[predictions];
  double L_sigma[predictions], L_error[predictions], L_fit[predictions];
  double T_sigma[predictions], T_error[predictions], T_fit[predictions];
  ...
  ofstream L_output_file("output/custom_differential_diffractive_L"+filename_end+".txt");
  ...
  ofstream T_output_file("output/custom_differential_diffractive_T"+filename_end+".txt");
