...
const double normalization = alpha_em*N_c*sigma_0*e_f*e_f/(8*gsl_pow_4(2*M_PI));
...
double L_integrand(double r, double z, double Q2, double x) {
  return r*sqrt(z*(1-z))*4*Q2*z*z*gsl_pow_2(1-z)*gsl_pow_2(gsl_sf_bessel_K0(epsilon(z, Q2)*r))*dipole_amplitude(r, x);
}


double T_integrand(double r, double z, double Q2, double x) {
  return r*sqrt(z*(1-z))*(m_f*m_f*gsl_pow_2(gsl_sf_bessel_K0(epsilon(z, Q2)*r)) 
  + epsilon2(z, Q2)*(z*z+gsl_pow_2(1-z))*gsl_pow_2(gsl_sf_bessel_K1(epsilon(z, Q2)*r)))*dipole_amplitude(r, x);
}
...
void integrate_for_L_sigma(thread_par_struct par) {
  ...
  double xl[2] = {0, 0};
  double xu[2] = {integration_radius, 1};
...
int main() {
  ...
  ofstream L_output_file("data/alternative_diff_L_sigma_x.txt");
  ...
  L_sigma_canvas->Print("figures/alternative_diff_L_sigma_x_distribution.pdf");
 

  TMultiGraph* T_graphs = new TMultiGraph();
  T_graphs->SetTitle("Diffractive transverse cross section;x;cross section (mb)");

  ofstream T_output_file("data/alternative_diff_T_sigma_x.txt");
  ...
  T_sigma_canvas->Print("figures/alternative_diff_T_sigma_x_distribution.pdf");
