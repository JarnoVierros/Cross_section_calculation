...
const int integration_calls = 10000000; //100000
...
double Phi_integrand(double r, double phi, double R, double PHI, double b, double theta, double P2, double y, double xpom) {
    ...
    double radius = r*r+R*R+b*b;
    //cout << radius << ", " << exp(-sqrt(radius)) << endl;
    return r*R*b*exp(-sqrt(radius));
...
void integrate(thread_par_struct par) {
    ...
    double xu[dim] = {range, 2*M_PI, range, 2*M_PI, range, 2*M_PI};
