...
double dipole_amplitude_from_xy(double x1, double x2, double y1, double y2, double z, double pT, double rapidity, double Q2) {
  double x = sqrt(x1*x1+x2*x2);
  double y = sqrt(y1*y1+y2*y2);
  double dip_r, dip_bmin, dip_phi;
  if (x < y) {
    dip_r = sqrt(gsl_pow_2(x1-y1)+gsl_pow_2(x2-y2));
    dip_bmin = x;
    dip_phi = acos((x1*(y1-x1)+x2*(y2-x2))/(x*dip_r));
  } else {
    dip_r = sqrt(gsl_pow_2(y1-x1)+gsl_pow_2(y2-x2));
    dip_bmin = y;
    dip_phi = acos((y1*(x1-y1)+y2*(x2-y2))/(y*dip_r));
  }
  return dipole_amplitude(z, dip_r, dip_bmin, dip_phi, pT, rapidity, Q2);
}

double T_integrand(double x1, double x2, double y1, double y2, double xbar1, double xbar2, double z, double Q2, double pT, double rapidity) {
  double r = sqrt(gsl_pow_2(x1-y1)+gsl_pow_2(x2-y2));
  double rbar = sqrt(gsl_pow_2(xbar1-y1)+gsl_pow_2(xbar2-y2));
  double x = sqrt(x1*x1+x2*x2);
  double y = sqrt(y1*y1+y2*y2);
  double xbar = sqrt(xbar1*xbar1+xbar2*xbar2);
  double dip_amp_1, dip_amp_2, dip_amp_3;
  dip_amp_1 = dipole_amplitude_from_xy(x1, x2, y1, y2, z, pT, rapidity, Q2);
  dip_amp_2 = dipole_amplitude_from_xy(y1, y2, xbar1, xbar2, z, pT, rapidity, Q2);
  dip_amp_3 = dipole_amplitude_from_xy(x1, x2, xbar1, xbar2, z, pT, rapidity, Q2);
  return cos(pT*(x1-xbar1))*(m_f*m_f*gsl_sf_bessel_K0(epsilon(z, Q2)*r)*gsl_sf_bessel_K0(epsilon(z, Q2)*rbar) + epsilon2(z, Q2)*(z*z + gsl_pow_2(1-z))*((x1-y1)*(xbar1-y1)+(x2-y2)*(xbar2-y2))/(r*rbar)*gsl_sf_bessel_K1(epsilon(z, Q2)*r)*gsl_sf_bessel_K1(epsilon(z, Q2)*rbar))*(dip_amp_1+dip_amp_2-dip_amp_3);
}
...
void integrate_for_T_sigma(thread_par_struct par) {
  ...
  double xl[dim] = {-100, -100, -100, -100, -100, -100, 0};
  double xu[dim] = {100, 100, 100, 100, 100, 100, 1};
