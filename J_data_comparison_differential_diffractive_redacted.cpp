...
const double x_limits[2] = {7e-5, 0.1};
...
*/
    ...
    comparison_graph->GetXaxis()->SetLabelSize(18);
    comparison_graph->GetYaxis()->SetLabelSize(16);

    comparison_graph->GetXaxis()->SetNdivisions(3);
    comparison_graph->GetYaxis()->SetNdivisions(6);

    comparison_graph->GetXaxis()->ChangeLabel(1, -1, 0, -1, -1, -1, -1);
    comparison_graph->GetXaxis()->ChangeLabel(-1, -1, 0, -1, -1, -1, -1);
  ...
  */
  ...
  for (int i=0; i<32; i++) {
    ...
    TString beta_string = "#it{#beta} = " + beta_stream.str();
    double Q2_x = 1.5e-4;
    double Q2_y = 0.083;
    ...
    TLatex* Q2_text = new TLatex(Q2_x, Q2_y, Q2_string);
    Q2_text->SetTextSize(14);
    Q2_text->Draw("Same");

    beta_stream << setprecision(2) << plots[i+offset].beta;
    TLatex* beta_text = new TLatex(beta_x, beta_y, beta_string);
    beta_text->SetTextSize(14);
    beta_text->Draw("Same");
