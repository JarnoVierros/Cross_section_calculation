...
int main() {
  ...
  comparison_graphs->SetTitle("ASD");

  for (int i=0; i<filecount; i++) {
    for (int j=0; j<Q2[i].size(); j++) {
      vector<double> temp = x[i][j];
      ...
      double sigma_error_arr[sigma_error[i][j].size()];
      zero_array(sigma_error_arr, sigma_error[i][j].size());

      TGraphErrors* subgraph = new TGraphErrors(x[i][j].size(), x_arr, sigma_arr, sigma_error_arr, sigma_error_arr);
      TString subgraph_name = "Q^{2}=" + to_string(Q2[i][j]);
  ...
  comparison_canvas->Print("figures/multiplot.pdf");
