...
int main() {
  ...
  L_graphs->SetTitle("Old longitudinal cross section;x;cross section (mb);cross section error (mb)");

  ofstream L_output_file("data/old_inclusive_L_sigma_x.txt");
  ...
  L_sigma_canvas->Print("figures/old_L_sigma_x_distribution.pdf");
  

  TMultiGraph* T_graphs = new TMultiGraph();
  T_graphs->SetTitle("Old Transverse cross section;x;cross section (mb)");

  ofstream T_output_file("data/old_inclusive_T_sigma_x.txt");
  ...
  TCanvas* T_sigma_canvas = new TCanvas("old_T_sigma_canvas", "", 1000, 600);
  ...
  T_sigma_canvas->Print("figures/old_T_sigma_x_distribution.pdf");
