...
const double normalization = 2*alpha_em*N_c*e_f*e_f;
...
double T_integrand(double r, double b, double z, double Q2, double W) {
  return r*b*(m_f*m_f*gsl_pow_2(gsl_sf_bessel_K0(epsilon(z, Q2)*r)) + epsilon2(z, Q2)*(z*z+gsl_pow_2(1-z))*gsl_pow_2(gsl_sf_bessel_K1(epsilon(z, Q2)*r)))*gsl_pow_2(dipole_amplitude(r, (Q2+4*m_f*m_f)/(W*W+Q2), b));
}
...
void integrate_for_T_sigma(thread_par_struct par) {
  ...
  double xl[dim] = {0, 0, 0};
  double xu[dim] = {r_limit, b_min_limit, 1};
...
int main() {
  ...
  */
  ...
  T_graphs->SetTitle("GBW diffractive transverse cross section;W (GeV);cross section (mb)");

  ofstream T_output_file("data/GBW_diff_LHC_T_sigma_W_"+dipole_amp_type+"_"+nucleus_type+".txt");
  ...
  TCanvas* T_sigma_canvas = new TCanvas("GBW_diff_T_sigma_canvas", "", 1000, 600);
  ...
  TString fig_filename = "figures/GBW_diff_LHC_T_sigma_W_"+dipole_amp_type+"_"+nucleus_type+".pdf";
